...
export function levelForVisits(visits: number, s: LoyaltySettings): Level {
  if (visits >= s.heisenberg_visits)
    return {
      symbol: "Hs",
      name: "Heisenberg",
      discount: s.heisenberg_discount,
      threshold: s.heisenberg_visits,
    };
  if (visits >= s.gold_visits)
    return { symbol: "Au", name: "Oro", discount: s.gold_discount, threshold: s.gold_visits };
  if (visits >= s.silver_visits)
    return { symbol: "Ag", name: "Plata", discount: s.silver_discount, threshold: s.silver_visits };
  if (visits >= s.bronze_visits)
    return {
      symbol: "Cu",
      name: "Bronce",
      discount: s.bronze_discount,
      threshold: s.bronze_visits,
    };
  return { symbol: "—", name: "Sin nivel", discount: 0, threshold: s.bronze_visits };
}

export function clientStatus(
  client: { crystals_expire_at: string | null; last_visit_at: string | null },
  s: Pick<LoyaltySettings, "inactivity_grace_days">,
) {
