...
type RegisterResult = { tx: Transaction; client: Client; services: string };
...
function RegisterPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data: s } = await supabase
        ...
        .order("display_order");
      setServices(s ?? []);
      const { data: ls } = await supabase.from("loyalty_settings").select("*").eq("id", 1).single();
      setSettings(ls);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const { data } = await supabase
        ...
        .select("*")
        .or(
          `full_name.ilike.%${clientQuery}%,phone.ilike.%${clientQuery}%,chem_id.ilike.%${clientQuery}%`,
        )
        .limit(8);
  ...
  const submit = async () => {
    if (!client || chosen.length === 0) {
      toast.error("Selecciona cliente y servicios");
      return;
    }
    setSubmitting(true);
    ...
    return (
      <Receipt
        ...
        onClose={() => {
          ...
          setSelected({});
          setRedeem(0);
          navigate({ to: "/register", search: { client: undefined } });
        }}
      />
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="mt-2 border border-border rounded-sm divide-y divide-border bg-popover">
                    {clientResults.map((c) => (
                      <button
                        key={c.id}
                        onClick={() => setClient(c)}
                        className="w-full text-left p-3 hover:bg-accent flex justify-between"
                      >
                        <span>
                          <span className="text-crystal font-display mr-2">{c.chem_id}</span>
                          {c.full_name}
                        </span>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    Visitas: {client.visits_count} · Cristales:{" "}
                    <span className={balance > 0 ? "text-crystal" : "text-danger"}>{balance}</span>
                    {level && level.discount > 0 && (
                      <>
                        {" "}
                        · Descuento de nivel: <span className="text-toxic">{level.discount}%</span>
                      </>
                    )}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {services.map((s) => {
                ...
                return (
                  <button
                    key={s.id}
                    onClick={() => setSelected({ ...selected, [s.id]: !on })}
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="text-xs uppercase tracking-widest text-muted-foreground">
                        {s.kind === "combo" ? "Combo" : s.kind === "base" ? "Base" : "Extra"}
                      </div>
          ...
          <HexPanel>
            ...
            {client && balance > 0 && (
              <div className="mt-4">
                ...
                />
                {cappedRedeem > 0 && (
                  <div className="text-toxic text-xs mt-1">
                    Descuento por cristales: {fmtMoney(redeemDisc)}
                  </div>
                )}
              </div>
            )}
        ...
        <HexPanel className="h-fit sticky top-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            {levelDiscPct > 0 && (
              <Row
                label={`Descuento ${levelDiscPct}%`}
                value={`− ${fmtMoney(levelDisc)}`}
                color="text-toxic"
              />
            )}
            {cappedRedeem > 0 && (
              <Row
                label={`Canje ${cappedRedeem} cristales`}
                value={`− ${fmtMoney(redeemDisc)}`}
                color="text-toxic"
              />
            )}
          ...
          <button
            onClick={submit}
            disabled={submitting || !client || chosen.length === 0}
            className="drum-button w-full py-3 mt-5 rounded-sm disabled:opacity-50"
          >
...
function Row({
  ...
  return (
    <div className="flex justify-between items-center">
      <span className={`uppercase tracking-widest text-xs ${color ?? "text-muted-foreground"}`}>
        {label}
      </span>
      <span className={`${color ?? ""} ${big ? "display text-3xl text-hazmat" : "font-semibold"}`}>
        {value}
      </span>
    </div>
  );
...
function Receipt({
  ...
  const msg = buildWhatsAppMessage({
    ...
    services,
    total: Number(tx.total_paid),
    crystals_earned: tx.crystals_earned,
    total_crystals: client.crystals_balance,
    expires_at: client.crystals_expire_at,
    chem_id: client.chem_id,
  });
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <HexPanel>
          <div className="display text-2xl text-crystal mb-3">
            {client.chem_id} · {client.full_name}
          </div>
          ...
          <Row
            label="Cristales totales"
            value={String(client.crystals_balance)}
            color="text-crystal"
          />
        ...
        <HexPanel>
          ...
          <div className="metal-panel rounded-sm p-3 text-sm whitespace-pre-wrap text-foreground">
            {msg}
          </div>
          <div className="flex gap-2 mt-4">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="drum-button flex-1 py-3 rounded-sm text-center text-sm flex items-center justify-center gap-2"
            >
              <Send className="w-4 h-4" /> Abrir WhatsApp
            </a>
            <button
              onClick={copy}
              className="metal-panel px-4 py-3 rounded-sm text-sm flex items-center gap-2"
            >
          ...
          <button
            onClick={onClose}
            className="w-full mt-3 py-2 text-xs uppercase tracking-widest text-muted-foreground hover:text-crystal flex items-center justify-center gap-2"
          >
            <Plus className="w-3 h-3" /> Nueva atención
          </button>
        </HexPanel>
      </div>
    </div>
