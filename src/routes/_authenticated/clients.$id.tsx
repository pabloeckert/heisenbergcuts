...
function ClientDetail() {
  ...
  const [history, setHistory] = useState<TransactionWithItems[]>([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      setHistory(t ?? []);
      const { data: s } = await supabase.from("loyalty_settings").select("*").eq("id", 1).single();
      setSettings(s);
    })();
  }, [id]);
  ...
  return (
    <div>
      <Link
        to="/clients"
        className="text-xs uppercase tracking-widest text-muted-foreground flex items-center gap-1 mb-3"
      >
      ...
      <div className="flex justify-between items-end mb-6 flex-wrap gap-4">
        <PageTitle
          subtitle={
            <span>
              <span className="text-crystal font-display text-lg">{client.chem_id}</span> ·{" "}
        ...
        <Link
          ...
          className="drum-button px-4 py-2.5 rounded-sm text-sm flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <HexPanel>
          ...
          <div className={`display text-5xl ${balance > 0 ? "text-crystal" : "text-danger"}`}>
            {balance}
          </div>
          <div className="text-xs text-muted-foreground mt-2">
            {client.crystals_expire_at
              ? `Vencen ${fmtDate(client.crystals_expire_at)}`
              : "Sin vencimiento"}
          </div>
          {balance === 0 && client.crystals_balance > 0 && (
            <div className="text-danger text-xs mt-1">
              Saldo {client.crystals_balance} expirado por inactividad
            </div>
          )}
          <div className="text-xs text-muted-foreground mt-3">
            ...
            {balance > 0 && (
              <span className="text-toxic">
                {" "}
                · disponible {fmtMoney(balance / settings.crystals_redeem_rate)}
              </span>
            )}
          </div>
        </HexPanel>
        <HexPanel>
          ...
          <div className="text-sm mt-3">
            Nivel actual: <span className="text-hazmat font-display text-lg">{level.name}</span> ·
            descuento <span className="text-toxic">{level.discount}%</span>
          </div>
        </HexPanel>
        <HexPanel>
          <div className="text-xs uppercase tracking-widest text-muted-foreground mb-2">
          ...
          {client.alias && (
            <div className="text-sm text-muted-foreground mt-2">Alias: {client.alias}</div>
          )}
          <div className="text-xs text-muted-foreground mt-2">
            Registrado: {fmtDate(client.created_at)}
          </div>
        </HexPanel>
      </div>
          ...
          <tbody>
            {history.map((t) => (
              <tr key={t.id} className="border-t border-border">
                ...
                <td className="p-3 text-right text-crystal">
                  +{t.crystals_earned} / −{t.crystals_redeemed}
                </td>
