...
type ServicePatch = TablesUpdate<"services">;
...
function ServicesPage() {
  ...
  const update = async (id: string, patch: ServicePatch) => {
  ...
  const addExtra = async (e: React.FormEvent) => {
    e.preventDefault();
    if (extras.length >= 3) {
      toast.error("Máximo 3 servicios extra");
      return;
    }
    const { error } = await supabase.from("services").insert({
      ...
      kind: "extra",
      price: newSvc.price,
      commission_pct: newSvc.commission_pct,
      display_order: 20 + extras.length,
    });
    if (error) toast.error(error.message);
  ...
  const renderGroup = (label: string, items: Service[]) => (
      ...
      <div className="space-y-2">
        {items.map((s) => (
          <ServiceRow
            key={s.id}
            s={s}
            onSave={(p) => update(s.id, p)}
            onDelete={!s.is_locked ? () => del(s.id) : undefined}
          />
        ))}
      </div>
    </HexPanel>
  ...
  return (
    <div>
      ...
      {renderGroup(
        "BASE (fijos)",
        services.filter((s) => s.kind === "base"),
      )}
      {renderGroup(
        "COMBOS",
        services.filter((s) => s.kind === "combo"),
      )}
        ...
        <HexPanel>
          ...
          <form onSubmit={addExtra} className="grid grid-cols-1 md:grid-cols-4 gap-2">
            <input
              ...
              className="bg-input border border-border px-3 py-2 rounded-sm md:col-span-2"
            />
            <input
              type="number"
              placeholder="Precio"
            ...
            <input
              ...
              value={newSvc.commission_pct}
              onChange={(e) => setNewSvc({ ...newSvc, commission_pct: Number(e.target.value) })}
              className="bg-input border border-border px-3 py-2 rounded-sm"
            />
            <button
              type="submit"
              className="drum-button col-span-full py-2 rounded-sm flex items-center justify-center gap-2"
            >
...
function ServiceRow({
  ...
  useEffect(() => {
    ...
    setComm(Number(s.commission_pct));
    setActive(s.active);
  }, [s.id]);
  const dirty =
    price !== Number(s.price) || comm !== Number(s.commission_pct) || active !== s.active;
  return (
    <div className="flex flex-wrap items-center gap-3 p-3 metal-panel rounded-sm">
      ...
      <label className="text-xs text-muted-foreground">
        ...
        <input
          ...
          className="block w-24 bg-input border border-border px-2 py-1 rounded-sm"
        />
      </label>
      <label className="text-xs text-muted-foreground">
        Comisión %
        <input
          ...
          className="block w-20 bg-input border border-border px-2 py-1 rounded-sm"
        />
      </label>
      ...
      {dirty && (
        <button
          onClick={() => onSave({ price, commission_pct: comm, active })}
          className="drum-button px-3 py-1.5 rounded-sm text-xs flex items-center gap-1"
        >
      ...
      {onDelete && (
        <button onClick={onDelete} className="text-danger hover:opacity-70">
