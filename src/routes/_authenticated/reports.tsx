...
function ReportsPage() {
  ...
  const load = async () => {
    const { data } = await (supabase as any).from("transactions")
      .select("*, transaction_items(*)").gte("created_at", since).order("created_at", { ascending: false });
    setTx(data ?? []);
    const { data: e } = await (supabase as any).from("expenses").select("*").gte("created_at", since).order("created_at", { ascending: false });
    setExpenses(e ?? []);
  ...
  const cash = tx.filter((t) => t.payment_method === "cash").reduce((s, t) => s + Number(t.total_paid), 0);
  ...
  tx.forEach((t) => { byBarber[t.barber_id] = (byBarber[t.barber_id] ?? 0) + Number(t.commission_total); });

  const addExp = async (e: React.FormEvent) => {
    e.preventDefault();
    const { error } = await (supabase as any).from("expenses").insert(newExp);
  ...
  const exportPdf = () => {
    const doc = new jsPDF();
    doc.setFontSize(18); doc.text("Heisenberg Cuts — Reporte Cash Flow", 14, 18);
    doc.setFontSize(10); doc.text(`Periodo: ${range} · desde ${fmtDate(since)}`, 14, 26);
    autoTable(doc, {
      startY: 32,
      head: [["Concepto", "Monto"]],
      body: [
    ...
    <div>
      <div className="flex justify-between items-end flex-wrap gap-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={exportPdf} className="drum-button px-4 py-2 rounded-sm flex items-center gap-2 text-xs"><Download className="w-4 h-4" /> PDF</button>
        </div>
      </div>
      ...
      <HexPanel className="mb-6">
        ...
        <table className="w-full text-sm">
          <thead className="text-[11px] uppercase tracking-widest text-muted-foreground">
            <tr><th className="text-left p-2">Barbero</th><th className="text-right p-2">Comisión</th></tr>
          </thead>
          <tbody>
            ...
            {Object.keys(byBarber).length === 0 && <tr><td colSpan={2} className="p-4 text-center text-muted-foreground">Sin datos.</td></tr>}
          </tbody>
        </table>
      </HexPanel>
      ...
      <HexPanel>
        ...
        <form onSubmit={addExp} className="grid grid-cols-1 md:grid-cols-4 gap-2 mb-4">
          <input required placeholder="Descripción" value={newExp.description} onChange={(e) => setNewExp({ ...newExp, description: e.target.value })} className="md:col-span-2 bg-input border border-border px-3 py-2 rounded-sm" />
          <input type="number" placeholder="Monto" required value={newExp.amount} onChange={(e) => setNewExp({ ...newExp, amount: Number(e.target.value) })} className="bg-input border border-border px-3 py-2 rounded-sm" />
          <input placeholder="Categoría" value={newExp.category} onChange={(e) => setNewExp({ ...newExp, category: e.target.value })} className="bg-input border border-border px-3 py-2 rounded-sm" />
          <button type="submit" className="drum-button col-span-full py-2 rounded-sm flex items-center justify-center gap-2 text-xs"><Plus className="w-4 h-4" /> Registrar gasto</button>
        </form>
        <table className="w-full text-sm">
          <thead className="text-[11px] uppercase tracking-widest text-muted-foreground">
            <tr><th className="text-left p-2">Fecha</th><th className="text-left p-2">Descripción</th><th className="text-left p-2">Categoría</th><th className="text-right p-2">Monto</th><th></th></tr>
          </thead>
          <tbody>
            {expenses.map((e) => (
              <tr key={e.id} className="border-t border-border">
                ...
                <td className="p-2"><button onClick={() => delExp(e.id)} className="text-danger"><Trash2 className="w-4 h-4" /></button></td>
              </tr>
            ))}
            {expenses.length === 0 && <tr><td colSpan={5} className="p-4 text-center text-muted-foreground">Sin gastos.</td></tr>}
          </tbody>
        </table>
      </HexPanel>
    </div>
