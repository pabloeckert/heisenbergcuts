...
function ClientsPage() {
  ...
  const [settings, setSettings] = useState<Pick<LoyaltySettingsRow, "inactivity_grace_days">>({
  ...
  const filtered = clients.filter((c) => {
    ...
    return (
      ...
      c.chem_id.toLowerCase().includes(term)
    );
  });
            ...
            {filtered.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-3">
                    {c.full_name}
                    {c.alias && <span className="text-muted-foreground text-xs"> · {c.alias}</span>}
                  </td>
                  ...
                  <td className={`p-3 ${st.color} text-xs uppercase tracking-widest`}>
                    ● {st.label}
                  </td>
                </tr>
              );
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <div>
              <label className="text-xs uppercase tracking-widest text-muted-foreground">
                Teléfono (WhatsApp con código país)
              </label>
              <input
                required
                value={form.phone}
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button type="submit" className="drum-button px-5 py-2.5 rounded-sm text-sm">
                Cocinar registro
              </button>
            </div>
          </form>
